...
namespace Here.WebApp.Configurations
{
  public static class IdentityDataInitializer
  {
    ...
    {
      if (roleManager
        .RoleExistsAsync(EnumsExtension.GetDescription(Roles.OVERWATCH))
        .Result)
      {
        ...
        if(userManager.FindByEmailAsync("").Result == null)
        {
          ...
          if (result.Succeeded)
            userManager.AddToRoleAsync(user, EnumsExtension.GetDescription(Roles.OVERWATCH)).Wait();
